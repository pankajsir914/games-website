...
export const useTPIN = () => {
  ...
  const { data: securityQuestion, refetch: refetchSecurityQuestion } = useQuery({
    queryKey: ['tpin-security-question'],
    queryFn: async () => {
      const { data, error } = await supabase.rpc('get_tpin_security_question');
      if (error) {
        console.error('Failed to get security question:', error);
        return { success: false, has_security_question: false } as SecurityQuestionResult;
      }
      return data as unknown as SecurityQuestionResult;
    },
    staleTime: 30000,
  });

  // Set TPIN mutation with security question
  const setTPINMutation = useMutation({
    mutationFn: async ({ tpin, securityQuestion, securityAnswer }: { 
      tpin: string; 
      securityQuestion?: string; 
      securityAnswer?: string;
    }) => {
      const { data, error } = await supabase.rpc('set_admin_tpin', { 
        p_tpin: tpin,
        p_security_question: securityQuestion || null,
    ...
    onSuccess: () => {
      ...
      queryClient.invalidateQueries({ queryKey: ['tpin-security-question'] });
      toast({
        title: 'TPIN Set Successfully',
        description: 'Your TPIN and security question have been configured.',
      });
    },
    onError: (error: Error) => {
      toast({
  ...
  // Verify security answer mutation
  const verifySecurityAnswerMutation = useMutation({
    mutationFn: async (answer: string) => {
      const { data, error } = await supabase.rpc('verify_tpin_security_answer', { p_answer: answer });
      if (error) throw error;
  ...
  return {
    ...
    // Security Question
    hasSecurityQuestion: securityQuestion?.has_security_question ?? false,
    securityQuestionText: securityQuestion?.security_question ?? null,
    
    // Actions
    setTPIN: setTPINMutation.mutate,
    verifyTPIN: verifyTPINMutation.mutateAsync,
    verifySecurityAnswer: verifySecurityAnswerMutation.mutateAsync,
    resetTPIN: resetTPINMutation.mutate,
    executeWithTPIN,
    refetchStatus,
    refetchSecurityQuestion,
    
    // Loading states
    isSettingTPIN: setTPINMutation.isPending,
    isVerifyingTPIN: verifyTPINMutation.isPending,
    isVerifyingSecurityAnswer: verifySecurityAnswerMutation.isPending,
