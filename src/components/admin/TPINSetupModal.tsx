...
import { Loader2, Shield, Eye, EyeOff, CheckCircle, RefreshCw } from 'lucide-react';
...
export const TPINSetupModal = ({ open, onOpenChange, canDismiss = false, mode = 'setup' }: TPINSetupModalProps) => {
  const { setTPIN, verifyTPIN, isSettingTPIN, isVerifyingTPIN, hasTPIN } = useTPIN();
  ...
  const [step, setStep] = useState<'verify' | 'new'>(mode === 'change' && hasTPIN ? 'verify' : 'new');

  const isChanging = mode === 'change';
  const isLoading = isSettingTPIN || isVerifyingTPIN;
  ...
  const resetForm = () => {
    ...
    setConfirmTpin('');
    setError('');
    setStep(isChanging && hasTPIN ? 'verify' : 'new');
  };

  const handleVerifyCurrentTpin = async () => {
    setError('');
    
    if (currentTpin.length < 4 || currentTpin.length > 6) {
      setError('TPIN must be 4-6 digits');
      return;
    }

    try {
      const result = await verifyTPIN(currentTpin);
      if (result.success) {
        setStep('new');
        setCurrentTpin('');
      } else {
      ...
      setError('Failed to verify TPIN');
    }
  };
      ...
      >
        <DialogHeader className="text-center">
          <div className="mx-auto mb-4 flex h-12 w-12 items-center justify-center rounded-full bg-primary/10">
            {isChanging ? <RefreshCw className="h-6 w-6 text-primary" /> : <Shield className="h-6 w-6 text-primary" />}
          </div>
          <DialogTitle>{isChanging ? 'Change Your TPIN' : 'Set Up Your TPIN'}</DialogTitle>
          <DialogDescription>
            {isChanging 
              ? step === 'verify' 
                ? 'Enter your current TPIN to verify your identity.'
        ...
        {isChanging && step === 'verify' && (
          <div className="space-y-4 mt-4">
            <div className="space-y-2">
              <Label htmlFor="currentTpin">Current TPIN</Label>
              <div className="relative">
                ...
                <button
                  ...
                  disabled={isLoading}
                >
                  {showCurrentTpin ? <EyeOff className="h-4 w-4" /> : <Eye className="h-4 w-4" />}
                </button>
              </div>
            </div>
            ...
            <Button 
              type="button" 
              className="w-full" 
              disabled={isLoading || currentTpin.length < 4}
              onClick={handleVerifyCurrentTpin}
            >
