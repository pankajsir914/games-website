...
export const WithdrawalTable = ({ filters }: WithdrawalTableProps) => {
  const { financeData, isLoading, processWithdrawalRequest, isProcessing } = useMasterAdminFinance();
  ...
  const withdrawalRequests = financeData?.withdrawal_requests || [];
              ...
              {filteredRequests.length > 0 ? (
                filteredRequests.map((request) => (
                  <TableRow key={request.id}>
                    ...
                    <TableCell>
                      <span className="font-semibold text-gaming-danger">₹{request.amount?.toLocaleString()}</span>
                    </TableCell>
                    <TableCell>
                        ...
                        <div className="space-y-1">
                          <div className="flex items-center gap-2">
                            <span className="text-xs text-muted-foreground">
                              {request.bank_details.account_holder || 'N/A'}
                            </span>
                            <Button 
                              variant="ghost" 
                              size="icon" 
                              className="h-6 w-6"
                              onClick={() => copyToClipboard(JSON.stringify(request.bank_details, null, 2))}
                            >
                              <Copy className="h-3 w-3" />
                            </Button>
                          </div>
                          <div className="text-xs text-muted-foreground">
                            {request.bank_details.bank_name || 'Bank N/A'} • ****{request.bank_details.account_number?.slice(-4) || '****'}
                          </div>
                        </div>
                      ) : (
