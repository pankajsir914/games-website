...
export const TransactionTable = ({ filters }: TransactionTableProps) => {
      ...
      <Card>
        <CardContent className="p-6">
          <div className="text-center text-destructive">
            <p>Error loading transactions: {error instanceof Error ? error.message : 'Unknown error'}</p>
          </div>
        </CardContent>
      </Card>
    );
  ...
  if (!isLoading && (!filteredTransactions || filteredTransactions.length === 0)) {
    return (
      <Card>
        <CardContent className="p-6">
          <div className="text-center text-muted-foreground">
            ...
            <p className="text-sm">
              {transactions && transactions.length === 0 
                ? "No transactions available. Transactions will appear here once users make deposits or withdrawals."
