      ...
      <div className="relative w-full h-20 sm:h-32 md:h-40 bg-gradient-to-br from-primary/5 to-secondary/10 flex items-center justify-center">
        {table.imageUrl ? (
          <img 
            ...
            loading="eager"
            onLoad={(e) => {
              console.log('✅ Image loaded successfully:', table.name, table.imageUrl);
            ...
            onError={(e) => {
              ...
              if (parent && parent.querySelector('img') === e.currentTarget) {
                const fallback = document.createElement('div');
                fallback.className = 'flex flex-col items-center justify-center gap-2 text-muted-foreground p-4';
                fallback.innerHTML = `
                  <div class="text-4xl">🎰</div>
                  <div class="text-xs text-center font-medium">${table.name}</div>
                `;
          ...
          <div className="flex flex-col items-center justify-center gap-2 text-muted-foreground p-4">
            ...
            <div className="text-xs text-center font-medium">{table.name}</div>
          </div>
        )}
        {isLive && (
