...
interface Joker1BettingBoardProps {
  bets: any[];
  locked?: boolean;
  min?: number;
  max?: number;
...
export const Joker1BettingBoard = ({
  bets = [],
  locked = false,
  min = 10,
  max = 100000,
        ...
        <CardContent className="space-y-4">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 sm:gap-4">
            {[
              ...
              return (
                <div key={label} className="border rounded-lg p-3 sm:p-4 bg-muted/30">
                  {/* Player Label */}
                  <div className="font-bold text-sm sm:text-base mb-3 text-center text-foreground">
                    {label}
                  </div>
                  
                  {/* Back and Lay Fields */}
                  <div className="grid grid-cols-2 gap-2">
                    {/* Back Field (Light Blue) */}
                    <button
                      onClick={() => handleBetClick(bet, bet?.nat || bet?.type || label, "back")}
                      disabled={backDisabled}
                      className={`h-[50px] sm:h-[60px] rounded flex items-center justify-center font-bold text-sm sm:text-base transition-all relative ${
                        backDisabled
                    ...
                    >
                      {backDisabled && (
                        <Lock className="w-3 h-3 sm:w-4 sm:h-4 absolute top-1 right-1" />
                      )}
