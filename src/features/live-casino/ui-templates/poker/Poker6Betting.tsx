...
interface Poker6BettingBoardProps {
  bets?: any[];
  locked?: boolean;
  min?: number;
  max?: number;
...
export const Poker6BettingBoard = ({
  ...
  // Debug: Log bets
  if (bets.length > 0) {
    console.log(`🎰 Poker6 - Total Bets: ${bets.length}, Player A: ${playerABets.filter(b => b.bet).length}, Player B: ${playerBBets.filter(b => b.bet).length}`);
  }
  ...
  // Poker6 Betting Cell Component - Responsive
  const Poker6Cell = ({ hand, bet }: { hand: string; bet: any }) => {
  ...
  return (
    <div className="space-y-4">
      {/* Poker6 Betting Table - Responsive: Stack on mobile, side-by-side on larger screens */}
        ...
        <div className="border-b-2 md:border-b-0 md:border-r-2 border-gray-400">
          <div className="grid grid-cols-3 gap-0">
            {playerABets.map(({ hand, bet }, idx) => (
              <Poker6Cell key={`player-a-${hand}-${idx}`} hand={hand} bet={bet} />
            ))}
          </div>
        </div>
        ...
        <div>
          <div className="grid grid-cols-3 gap-0">
            {playerBBets.map(({ hand, bet }, idx) => (
              <Poker6Cell key={`player-b-${hand}-${idx}`} hand={hand} bet={bet} />
            ))}
          </div>
        </div>
      </div>
