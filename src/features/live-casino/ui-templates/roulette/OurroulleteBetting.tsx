...
interface OurroulleteBettingProps {
  betTypes: any[];
...
// Check if bet is suspended (s=0 or b=0)
const isSuspended = (bet: any) => {
  if (!bet) return true;
  // API uses 's' field: 0 = suspended, 1 = open
  if (bet.s === 0 || bet.s === "0") return true;
  // Also check if odds are 0
  const odds = bet?.b ?? bet?.back ?? bet?.b1 ?? bet?.odds ?? 0;
...
export const OurroulleteBetting = ({
  betTypes = [],
  onPlaceBet,
  loading = false,
}: OurroulleteBettingProps) => {
  ...
  const NumberCell = ({ num }: { num: number }) => {
    ...
    const suspended = bet ? isSuspended(bet) : true; // If bet not found, consider suspended
  ...
  const ZeroCell = () => {
    ...
    const suspended = bet ? isSuspended(bet) : true;
    const odds = formatOdds(getOdds(bet));
  ...
  const ColumnCell = ({ columnNum }: { columnNum: number }) => {
    ...
    const bgClass = suspended ? "bg-green-200" : "bg-blue-600 hover:bg-blue-700";
    ...
    return (
      <button
        ...
        className={`
          ...
          font-bold text-xs rounded transition-all
          ${bgClass}
          ${suspended ? "cursor-not-allowed" : "cursor-pointer"}
        `}
      >
        {/* Content - always visible */}
        <div className="relative z-10 flex flex-col items-center justify-center w-full h-full">
          <span className={suspended ? "text-gray-700 font-bold" : "text-red-500 font-bold"}>2to1</span>
          <span className={`text-[10px] mt-0.5 ${suspended ? "text-gray-600" : "text-white"}`}>
            {odds}
          </span>
        </div>
        {/* Lock overlay - only when suspended */}
  ...
  const CategoryCell = ({ label }: { label: string }) => {
    ...
    return (
      ...
      >
        {/* Content - always visible */}
        <div className="relative z-10 flex flex-col items-center justify-center w-full h-full">
          <span className={`font-bold text-sm ${suspended ? "text-gray-700" : "text-red-600"}`}>
            {label}
          </span>
  ...
  // Get bets
  const zeroBet = find(betTypes, "0");
  const column1Bet = find(betTypes, "1st Column");
  const column2Bet = find(betTypes, "2nd Column");
  const column3Bet = find(betTypes, "3rd Column");
  const dozen1Bet = find(betTypes, "1st 12");
  const dozen2Bet = find(betTypes, "2nd 12");
  const dozen3Bet = find(betTypes, "3rd 12");
  const lowBet = find(betTypes, "1 to 18");
  ...
  const highBet = find(betTypes, "19 to 36");

  return (
    <>
      {/* ================= HEADER ================= */}
      <div className="flex justify-between items-center mb-4">
        <h3 className="text-sm font-semibold">Unique Roulette</h3>
      ...
      <Dialog open={rulesOpen} onOpenChange={setRulesOpen}>
        <DialogContent className="max-w-md text-sm max-h-[80vh] overflow-y-auto">
          <DialogHeader>
            <DialogTitle>Unique Roulette Rules</DialogTitle>
          </DialogHeader>
          ...
          <div className="space-y-3 text-xs leading-relaxed">
            <div>
              <p>
                ...
                numbers in the deck are open for bet in every round and odds are dynamics 
                based on numbers left in the deck.
              </p>
            </div>
            ...
            <div>
              ...
              <ul className="list-disc pl-4 space-y-1">
                ...
                <li><strong>Street Bet</strong> — place your chip at the end of any row of numbers. A Street Bet covers remaining numbers on that Street.</li>
                <li><strong>Corner Bet</strong> — place your chip at the corner (central intersection) where four numbers meet. All remaining numbers on that corner are covered.</li>
                <li><strong>Line Bet</strong> — place your chip at the end of two rows on the intersection between the two rows. A line bet covers all the remaining numbers in both rows.</li>
              </ul>
            </div>
            ...
            <div>
              ...
              <ul className="list-disc pl-4 space-y-1">
                <li><strong>Column Bet</strong> — place your chip in one of the boxes marked "2 to 1" at the end of the column that covers all remaining numbers in that column. The zero is not covered by any column bet.</li>
                <li><strong>Dozen Bet</strong> — place your chip in one of the three boxes marked "1st 12," "2nd 12" or "3rd 12" to cover the remaining numbers alongside the box.</li>
                <li><strong>Red/Black</strong> — place your chip in the Red or Black box to cover the all remaining red or all remaining black numbers. The zero is not covered by these bets.</li>
                <li><strong>Even/Odd</strong> — place your chip in one of these boxes to cover the remaining even or remaining odd numbers. The zero is not covered by these bets.</li>
                <li><strong>1-18/19-36</strong> — place your chip in either of these boxes to cover the first or second set of remaining numbers. The zero is not covered by these bets.</li>
              </ul>
            </div>
