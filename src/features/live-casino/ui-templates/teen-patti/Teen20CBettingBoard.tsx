...
interface Teen20CBettingBoardProps {
  bets: any[];
  locked?: boolean;
  min?: number;
  max?: number;
...
export const Teen20CBettingBoard = ({
  bets = [],
  locked = false,
  min = 10,
  max = 100000,
  ...
  const renderSuitCell = (bet: any, suits: string[], label: string, disabled: boolean) => {
    ...
    return (
      <button
        onClick={() => handleBetClick(bet, bet?.nat || bet?.type || label, "back")}
        disabled={isDisabled}
        className={`h-[50px] rounded flex items-center justify-center gap-2 font-bold text-sm transition-all ${
          isDisabled
            ? "bg-gray-700 text-gray-400 cursor-not-allowed"
            : "bg-blue-400 hover:bg-blue-500 text-white cursor-pointer shadow-md"
        }`}
      >
        <div className="flex gap-1">
          {suits.map((suit, idx) => (
            <span
              key={idx}
              className={`text-lg ${suit === "♠" || suit === "♣" ? "text-black" : "text-red-500"}`}
        ...
        {isDisabled && <Lock className="w-4 h-4 ml-1" />}
        <span className="ml-1">{backOdds === "0.00" ? "0" : backOdds}</span>
      </button>
    );
  };
            ...
            {[
              ...
              return (
                <div key={label} className="border rounded-lg overflow-hidden bg-muted/20">
                  {/* Header */}
                  <div className="bg-slate-800 text-white font-bold text-center py-2 px-3">
                    {label}
                  </div>
