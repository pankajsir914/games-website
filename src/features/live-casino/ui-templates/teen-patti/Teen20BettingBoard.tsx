...
interface Teen20BettingBoardProps {
  bets: any[];
  locked?: boolean;
  min?: number;
  max?: number;
...
export const Teen20BettingBoard = ({
  bets = [],
  locked = false,
  min = 10,
  max = 100000,
  ...
  const renderSuitCell = (bet: any, suits: string[], label: string, disabled: boolean) => {
    ...
    return (
      <button
        onClick={() => handleBetClick(bet, bet?.nat || bet?.type || label, "back")}
        disabled={isDisabled}
        className={`h-[40px] sm:h-[50px] rounded flex items-center justify-center gap-1 sm:gap-2 font-bold text-xs sm:text-sm transition-all ${
          isDisabled
            ? "bg-gray-700 text-gray-400 cursor-not-allowed"
            : "bg-blue-400 hover:bg-blue-500 text-white cursor-pointer shadow-md"
        }`}
            ...
            {[
              ...
              return (
                <div key={label} className="border rounded-lg overflow-hidden bg-muted/20">
                  {/* Header */}
                  <div className="bg-slate-800 text-white font-bold text-center py-1.5 sm:py-2 px-2 sm:px-3 text-xs sm:text-sm">
                    {label}
                  </div>
