...
DO $$ BEGIN
  CREATE POLICY "Public can read admin payment QR images"
  ON storage.objects
  FOR SELECT
  USING (bucket_id = 'admin-payment-qr');
EXCEPTION WHEN duplicate_object THEN NULL; END $$;

-- Admins can upload/update/delete their own folder objects
DO $$ BEGIN
  CREATE POLICY "Admins can upload admin QR images"
  ON storage.objects
  FOR INSERT
  WITH CHECK (
    bucket_id = 'admin-payment-qr'
...
DO $$ BEGIN
  CREATE POLICY "Admins can update their admin QR images"
  ON storage.objects
  FOR UPDATE
  USING (
    bucket_id = 'admin-payment-qr'
...
DO $$ BEGIN
  CREATE POLICY "Admins can delete their admin QR images"
  ON storage.objects
  FOR DELETE
  USING (
    bucket_id = 'admin-payment-qr'
