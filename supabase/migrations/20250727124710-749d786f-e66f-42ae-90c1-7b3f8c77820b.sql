...
DO $$
BEGIN
  IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'ludo_rooms') THEN
    -- Drop old constraint if it exists
    ALTER TABLE public.ludo_rooms DROP CONSTRAINT IF EXISTS ludo_rooms_entry_fee_check;
    -- Add new constraint
    ALTER TABLE public.ludo_rooms ADD CONSTRAINT ludo_rooms_entry_fee_check 
      CHECK (entry_fee >= 10.00 AND entry_fee <= 1000.00);
  END IF;
END $$;
