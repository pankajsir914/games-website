-- Add created_by field to profiles to track which admin created the user
...
-- Update the get_users_management_data function to support admin filtering
...
BEGIN
  ...
  WITH user_data AS (
    ...
    LEFT JOIN (
      ...
      WHERE type = 'credit' AND reason LIKE '%deposit%'
      GROUP BY user_id
    ) deposits ON deposits.user_id = u.id
    LEFT JOIN (
      SELECT user_id, SUM(amount) as total
      FROM wallet_transactions
      WHERE type = 'debit' AND reason LIKE '%withdrawal%'
      GROUP BY user_id
