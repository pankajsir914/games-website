...
CREATE TABLE public.chicken_run_rounds (
  id UUID NOT NULL DEFAULT gen_random_uuid() PRIMARY KEY,
  round_number INTEGER NOT NULL UNIQUE,
  trap_positions JSONB NOT NULL DEFAULT '[]'::jsonb,
  difficulty_level TEXT NOT NULL DEFAULT 'medium',
  status TEXT NOT NULL DEFAULT 'betting',
  bet_start_time TIMESTAMP WITH TIME ZONE NOT NULL DEFAULT NOW(),
  bet_end_time TIMESTAMP WITH TIME ZONE NOT NULL DEFAULT (NOW() + INTERVAL '30 seconds'),
  created_at TIMESTAMP WITH TIME ZONE NOT NULL DEFAULT NOW(),
  updated_at TIMESTAMP WITH TIME ZONE NOT NULL DEFAULT NOW()
);

-- Create chicken_run_bets table
CREATE TABLE public.chicken_run_bets (
  id UUID NOT NULL DEFAULT gen_random_uuid() PRIMARY KEY,
  user_id UUID NOT NULL,
  round_id UUID NOT NULL REFERENCES public.chicken_run_rounds(id),
  bet_amount NUMERIC(10,2) NOT NULL,
  tiles_revealed JSONB NOT NULL DEFAULT '[]'::jsonb,
...
CREATE TABLE public.chicken_run_leaderboard (
  id UUID NOT NULL DEFAULT gen_random_uuid() PRIMARY KEY,
  user_id UUID NOT NULL UNIQUE,
  total_games INTEGER NOT NULL DEFAULT 0,
  total_won INTEGER NOT NULL DEFAULT 0,
  total_lost INTEGER NOT NULL DEFAULT 0,
  highest_multiplier NUMERIC(10,2) NOT NULL DEFAULT 0,
...
CREATE POLICY "Anyone can view chicken run rounds"
ON public.chicken_run_rounds FOR SELECT
USING (true);

-- RLS Policies for bets
CREATE POLICY "Users can view their own chicken run bets"
ON public.chicken_run_bets FOR SELECT
USING (auth.uid() = user_id);

CREATE POLICY "Users can create their own chicken run bets"
ON public.chicken_run_bets FOR INSERT
WITH CHECK (auth.uid() = user_id);

-- RLS Policies for leaderboard (public read)
CREATE POLICY "Anyone can view chicken run leaderboard"
ON public.chicken_run_leaderboard FOR SELECT
USING (true);

-- Create function to place a chicken run bet
CREATE OR REPLACE FUNCTION public.place_chicken_run_bet(
...
-- Create function to reveal a tile
CREATE OR REPLACE FUNCTION public.reveal_chicken_run_tile(
  p_bet_id UUID,
  p_row INTEGER,
  p_column INTEGER
) RETURNS JSONB
...
-- Create function to cash out
CREATE OR REPLACE FUNCTION public.cashout_chicken_run_bet(
  p_bet_id UUID
) RETURNS JSONB
LANGUAGE plpgsql
SECURITY DEFINER
...
CREATE INDEX idx_chicken_run_bets_user_id ON public.chicken_run_bets(user_id);
CREATE INDEX idx_chicken_run_bets_round_id ON public.chicken_run_bets(round_id);
CREATE INDEX idx_chicken_run_rounds_status ON public.chicken_run_rounds(status);
CREATE INDEX idx_chicken_run_leaderboard_highest_multiplier ON public.chicken_run_leaderboard(highest_multiplier DESC);