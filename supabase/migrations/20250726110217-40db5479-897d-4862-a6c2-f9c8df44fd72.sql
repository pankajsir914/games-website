...
BEGIN
  ...
  FOR inactive_player IN 
    SELECT pp.id, pp.table_id, pp.user_id
  ...
  LOOP
    ...
    IF inactive_player.chip_count > 0 THEN
      PERFORM update_wallet_balance(
        ...
        (SELECT chip_count FROM poker_players WHERE id = inactive_player.id),
        'credit',
...
ALTER PUBLICATION supabase_realtime ADD TABLE poker_games;
...
ALTER TABLE poker_players REPLICA IDENTITY FULL;
ALTER TABLE poker_games REPLICA IDENTITY FULL;
