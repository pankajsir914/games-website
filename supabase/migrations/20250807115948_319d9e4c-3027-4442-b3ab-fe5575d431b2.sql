-- Fix foreign key constraint issue and create proper master admin setup

-- First, remove the user_roles record that's causing the constraint issue
DELETE FROM public.user_roles WHERE user_id = '00000000-0000-0000-0000-000000000001';

-- Now remove other records
...
-- Add RLS policy to allow authenticated users to check roles more broadly
CREATE POLICY "Users can check master admin roles" 
ON public.user_roles 
FOR SELECT 
USING (auth.uid() IS NOT NULL);

-- Create a function to properly set up master admin after user signup
