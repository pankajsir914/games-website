...
export interface Roulette11Result {
  number: number;
...
export function isRoulette11Table(tableId: string): boolean {
  return tableId?.toLowerCase() === "roulette11";
}
...
export function parseRoulette11Result(win: string): Roulette11Result {
  const num = parseInt(win, 10);

  const result: Roulette11Result = {
    number: num,
    color: "green",
    oddEven: null,
    range: null,
  ...
  console.log("🎯 [Roulette11 Parsed]", result);
  return result;
}
...
export function isWinningRoulette11Bet(
  betNat: string,
  result: Roulette11Result
): boolean {
  const bet = betNat.toLowerCase().trim();

  // Split / Street / Corner bets
  if (bet.includes(",")) {
    return bet
      .split(",")
...
export function formatRoulette11Last10(res: any[]) {
  return res.map(r => ({
    mid: r.mid,
    result: r.win, // "22", "08", "01"
  }));
}
