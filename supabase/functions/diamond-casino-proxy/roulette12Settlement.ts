...
export interface Roulette12Result {
  number: number;
...
export function isRoulette12Table(tableId: string): boolean {
  return tableId?.toLowerCase().startsWith("roulette");
}
...
export function parseRoulette12Result(win: string): Roulette12Result {
  const num = parseInt(win, 10);

  const result: Roulette12Result = {
    number: num,
    color: "green",
    oddEven: null,
    range: null,
  ...
  console.log("🎯 [Roulette12 Parsed]", result);
  return result;
}
...
export function isWinningRoulette12Bet(
  betNat: string,
  result: Roulette12Result
): boolean {
...
export function formatRoulette12Last10(res: any[]) {
  return res.map(r => ({
    mid: r.mid,
    result: r.win,
  }));
